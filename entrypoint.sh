...
DB_USER="${POSTGRES_USER:-test_user}" # <<< Match docker-compose/db_utils default
DB_NAME="${POSTGRES_DB:-umls_postgres_db}" # <<< Match docker-compose/db_utils default
...
MRCONSO_FILE="${UMLS_META_DIR}/MRCONSO.RRF" # Renamed for clarity
MRSTY_FILE="${UMLS_META_DIR}/MRSTY.RRF"     # Renamed for clarity
MRREL_FILE="${UMLS_META_DIR}/MRREL.RRF"     # <<< Added MRREL file path
...
check_db_populated() {
    ...
    if [ $exit_code -eq 0 ] && [ -n "$query_output" ]; then
        ...
        # Could indicate an incomplete previous load, maybe safer not to create it here.
...
# <<< MODIFIED to check for MRREL.RRF >>>
check_source_files_exist() {
    ...
    if [ ! -f "$MRREL_FILE" ]; then
        echo "  ERROR: Missing required source file: $MRREL_FILE" # <<< Added check for MRREL
        all_found=false
    fi
    ...
    if check_source_files_exist; then
        ...
        if [ $load_exit_code -eq 0 ]; then
            ...
            if [ -f "$LOAD_SUCCESS_FLAG_FILE" ]; then
                 ...
                 echo "         Load might be incomplete. Check python script logs carefully." >&2

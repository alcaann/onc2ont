...
from typing import Any, Dict, List, Optional, Callable # Ensure Any is imported
...
class BaseProcessingPipeline(ABC):
    ...
    def process(self, phrase: str, log_func: Optional[Callable[[str], None]] = None) -> Dict[str, List[Dict[str, Any]]]:
        ...
        Returns:
            A dictionary containing lists of extracted concepts and relations.
            Expected structure:
            {
                'concepts': [
                    {
                        ...
                        'sem_types': List[str],# List of semantic types (TUI or Name) from target SAB (e.g., NCI)
                        'score': Optional[float], # Confidence score if available (often None for cTAKES)
